    ...
    for ORDERING in "${ORDERINGS[@]}" ; do
      TESTING_PATH="$CONTAINER/$ORDERING/$SORT"
        ...
        if [ "$TEST_ITERATOR_METRICS" == true ] ; then
          #$ITR_PATH="data/itrdata/$SORT/$CONTAINER/$ORDERING/$LENGTH.tsv"
        ...
        if [ "$TEST_CPU_AND_MEMORY" == true ] ; then
          ...
          RUN_PATH_CPU="data/rundata/$TESTING_PATH/cpu_points.dat"
          RUN_PATH_MEM="data/rundata/$TESTING_PATH/mem_points.dat"
          printf 'LENGTH\t%s\n' "$SORT" > "$RUN_PATH_CPU"
          printf 'LENGTH\t%s\n' "$SORT" > "$RUN_PATH_MEM"
        fi
        if [ "$TEST_TIME" == true ] ; then
          TIME_PATH="data/timedata/$TESTING_PATH/points.dat"
          printf 'LENGTH\t%s\n' "$SORT" > "$TIME_PATH"
        fi
        if [ "$TEST_CALLGRIND" == true ] ; then
          CPU_PATH="data/cpudata/$TESTING_PATH/points.dat"
          printf 'LENGTH\t%s\n' "$SORT" > "$CPU_PATH"
        fi
        if [ $TEST_PERF == true ] ; then
          #echo "NOT IMPLEMENTED"
          PERF_PATH="data/ipc/$TESTING_PATH/points.dat"
          printf 'LENGTH\t%s\n' "$SORT" > "$PERF_PATH"
        fi
      done
  done
    ...
    for ORDERING in "${ORDERINGS[@]}" ; do
      ...
      for LENGTH in "${LENGTHS[@]}" ; do
        ...
        if [ "$TEST_CPU_AND_MEMORY" == true ] ; then
          #Create pmem.dat and pcpu.dat, Peak Memory and Percentage CPU resp.
          #pull apart the columns.  Avg. % CPU first, then peak memory
          INPUT_PATH="data/rundata/$TESTING_PATH/$LENGTH.tsv"
          OUTPUT_CPU="data/rundata/$TESTING_PATH/cpu_points.dat"
          OUTPUT_MEM="data/rundata/$TESTING_PATH/mem_points.dat"
          CPU_MEDIAN="$(median "$(awk '{print $1}' "$INPUT_PATH" | sed 's/[\%]//g' )" )"
          MEM_MEDIAN="$(median "$(awk '{print $2}' "$INPUT_PATH" )" )"
          printf '%d\t%d\n' "$LENGTH" "$( echo "$CPU_MEDIAN" | sed 's/[\%]//g' )" >> "$OUTPUT_CPU"
          printf '%d\t%d\n' "$LENGTH" "$MEM_MEDIAN" >> "$OUTPUT_MEM"
        fi
        if [ "$TEST_TIME" == true ] ; then
          RUN_PATH="data/timedata/$TESTING_PATH/$LENGTH.tsv"
          OUTPUT_PATH="data/timedata/$TESTING_PATH/points.dat"
          MEDIAN="$(median "$(tr '\t' '\n' < "$RUN_PATH" )" )"
          printf '%d\t%d\n' "$LENGTH" "$MEDIAN" >> "$OUTPUT_PATH"
        fi
        #if [ $TEST_CALLGRIND == true ] ; then
          #TODO: NOT IMPLEMENTED
          #$CPU_PATH="data/cpudata/$TESTING_PATH/$LENGTH.tsv"
          #$OUTPUT_PATH="data/cpudata/$TESTING_PATH/points.dat"

        #fi
        if [ "$TEST_PERF" == true ] ; then
          #TODO: NOT IMPLEMENTED
          INPUT_PATH="data/ipc/$TESTING_PATH/$LENGTH.tsv"
          OUTPUT_PATH="data/ipc/$TESTING_PATH/points.dat"
          IPC_MEDIAN="$(median "$(awk '{print $1}' "$INPUT_PATH" )" )"
          printf '%d\t%.4f\n' "$LENGTH" "$IPC_MEDIAN" >> "$OUTPUT_PATH"
        fi
      done
    done
  done
...
for CONTAINER in "${CONTAINERS[@]}" ; do
  for ORDERING in "${ORDERINGS[@]}" ; do
    ...
    for SORT in "${SORTS[@]}" ; do
      TESTING_PATH="$CONTAINER/$ORDERING/$SORT"
      if [ "$TEST_ITERATOR_METRICS" == true ] ; then
        ITR_PATH+=("data/itrdata/$TESTING_PATH/points.dat")
      fi
      if [ "$TEST_CPU_AND_MEMORY" == true ] ; then
        CPU_PATH+=("data/rundata/$TESTING_PATH/cpu_points.dat")
        MEM_PATH+=("data/rundata/$TESTING_PATH/mem_points.dat")
      fi
      if [ "$TEST_TIME" == true ] ; then
        TIME_PATH+=("data/timedata/$TESTING_PATH/points.dat")
      fi
      if [ "$TEST_CALLGRIND" == true ] ; then
        CACHE_PATH+=("data/cpudata/$TESTING_PATH/points.dat")
      fi
      if [ "$TEST_PERF" == true ] ; then
        IPC_PATH+=("data/ipc/$TESTING_PATH/points.dat")
      fi
    done
    ...
    if [ "$TEST_ITERATOR_METRICS" == true ] ; then
      DATA_PATH="data/itrdata/$COMPILED_TESTS_PATH/compiled_points.dat"
    ...
    if [ "$TEST_CPU_AND_MEMORY" == true ] ; then
      #CPU_DATA="data/rundate/$COMPILED_TESTS_PATH/cpu_compiled_points.dat"
      MEM_DATA="data/rundata/$COMPILED_TESTS_PATH/mem_compiled_points.dat"
    ...
    if [ "$TEST_TIME" == true ] ; then
      DATA_PATH="data/timedata/$COMPILED_TESTS_PATH/compiled_points.dat"
    ...
    if [ "$TEST_CALLGRIND" == true ] ; then
      DATA_PATH="data/cpudata/$COMPILED_TESTS_PATH/compiled_points.dat"
    ...
    if [ $TEST_PERF == true ] ; then
      #TODO: NOT IMPLEMENTED
      DATA_PATH="data/ipc/$COMPILED_TESTS_PATH/compiled_points.dat"
      recursive_join "${IPC_PATH[@]}" > "$DATA_PATH"
      TITLE="Instruction Per Cycle Per Sort on $ORDERING"
      XLABEL="Number of Elements"
      YLABEL="Instructions Per Cycle"
      SAVE_PATH="IPC_$ORDERING""_""$CONTAINER.eps"
      plot_wrapper "$DATA_PATH" "$TITLE" "$XLABEL" "$YLABEL" "$SAVE_PATH"
    fi
  done
done
